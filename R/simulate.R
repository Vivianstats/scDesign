estimate_pa = function(rcount, ncores = 10){
  ...
  rcount = sweep(rcount, 2, 1e6/colsums, FUN = "*")
  ...
  cell_drate = colSums(rcount == 0)/ngenes
  maxlog = max(lcount)
...
simulate_ofo = function(estpa, Js, S = NULL){
  ...
  simu_trans = sweep(simu_trans, 2, simu_sj/1e6, FUN = "*")
...
simulate_de_mfo = function(rcount, estpa, Js, ngroup = 2, S = 1e8, 
                                 pUp = 0.05, pDown = 0.05,
                                 fU = 5, fL = 3){
  ...
  res = lapply(1:ngroup, function(cc){
    ...
    ### print("introduce dropouts")
    sim_cell_drate = sample(cell_drate, Js, replace = TRUE)
    ...
    simucount[pa_rowind, ] = simu_multi
    rownames(simucount) = paste0("gene", 1:ngenes)
    colnames(simucount) = paste0("C", cc, "_", 1:Js)
    return(simucount)
  })
